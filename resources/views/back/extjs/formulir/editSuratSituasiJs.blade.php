...
<script>
  ...
  function load() {
    ...
    $.ajax({
      type: "GET",
      url: "{{ url('data/surat-situasi-tanah') }}" + "/" + "{{ request()->route('id') }}",
      dataType: "json",
  ...
  $(document).ready(function() {
    ...
    $('#formsuratsituasi').submit(function(e) {
      e.preventDefault();
      var formData = new FormData($(this)[0]);
      $.ajax({
        type: "POST",
        url: "{{ url('formulir/surat-situasi-tanah') }}" + "/" + "{{ request()->route('id') }}" + "/update",
        data: formData,
        dataType: "json",
        contentType: false,
        processData: false,
        ...
        success: function(res) {
          ...
          if (res.ok) {
            ...
            $('.card-body').html(res.message + ' <a href="{{ url('formulir/surat-situasi-tanah') }}">Ajukan baru</a>');
          } else {
            iziToast.error({
              title: 'Error!',
              message: "somthing wnet wrong",
